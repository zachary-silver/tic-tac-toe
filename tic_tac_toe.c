...
int main(void)
{
    ...
    /* If player wants to play against AI, randomly */
    /* decide if auto is player1 or player2. */
    input = tolower(input);
    if (input == 'y') {
        auto_player = rand() % 2;
    } else {
    ...
    /* Randomly assign which player goes first and what team they're on. */
    turn = rand() % 2;
    while (!match_over) {
        if (rand() % 2) {
            player1_team = 'x';
...
void display_board(char board[ROWS][COLS])
{
    for (int i = 0; i < ROWS; i++) {
        ...
        /* Don't want to print a line after the last row. */
        if (i < ROWS - 1) {
            printf("\n");
            draw_tabs(TABS);
            printf("-----------");
